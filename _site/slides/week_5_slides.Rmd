...
Today we're going to talk about our first true machine learning algorithm, the 
...
The last thing I want to point out before we start building trees is 
that decision trees are almost never the best choice for a problem. 
...
package. The syntax is almost exactly like `lm` -- we just provide a 
...
I've printed this out on the next slide.
...
can see that `rpart` assigns this node "No". 

Below the label, we can see the 
probability of a data point in this node being a "Yes" (17%) and the percentage 
...
(This graph and the next one are from the course textbook 
...
the percentage of your bin that is "Yes". 

This means that our tree, which has
