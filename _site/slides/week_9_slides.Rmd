...
We're now able to specify what pre-processing steps we want to take using any of
the `step_` functions from `recipes`. To say that we want to dummy encode all
...
With our variables now encoded, we can go ahead and split our data into training
and testing:
...
We can then calculate RMSE as normal:
...
k_fold_cv <- function(data, k, ...) {
  ...
  vapply(
    ...
    \(fold_idx) {
      ...
      xtrain <- as.matrix(fold_train[setdiff(names(fold_train), "Sale_Price")])
...
But this is a 1-credit class, and I simply don't have the space to walk through
all available hyperparameters. If you're using `lightgbm` or `xgboost` 
professionally and want more information, I highly recommend [this website (link) on available parameters](https://sites.google.com/view/lauraepp/parameters). 
...
```{r}
xtest <- as.matrix(stacked_test[setdiff(names(stacked_test), "Sale_Price")])
...
```{r}
stacked_test$avg <- (stacked_test$lgb + stacked_test$rf + stacked_test$gbm) / 3
```

Conceptually, doing this is a way of saying "all of these models are equally 
likely to be the best one". 

Pragmatically, even this simple ensemble beats all but our GBM on the test data:
...
This vector now represents the proportion that each model will contribute to our 
final prediction. Intuitively enough, GBM here has a somewhat larger weight and 
the random forest has had its weight reduced from the baseline 1/3.
...
And this model-weighted ensemble outperforms both the averages!
...
Even when they aren't your outright best model -- such as our
example here, where GBM is still on top -- stacked ensembles are a useful way to 
make your predictions more robust on unknown data. 

It might be the case that this GBM 
is always a better model in the situations we'll use it in, or it might be the 
case that with different test data the random forest or lightgbm model would 
take the lead.
...
and computational power it requires. But of course, where exactly that point is 
