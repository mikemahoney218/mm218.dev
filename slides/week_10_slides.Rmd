...
# k-Nearest Neighbors and Kernel-Based ML
...
(or **kernel machines**). However, I don't necessarily want to spend a ton of
time getting into the math behind what a "kernel" is.
...
The method we'll walk through today is called **k-nearest neighbors** (KNN), 
...
so we'll walk through them here.
...
For instance, if k = 1 our model would look at the closest point (attached by
...
For KNN, k is a hyperparameter which we'll pick through tuning. We could 
imagine, for instance, setting it to 2, in which case KNN would look at the two
closest plots and still predict edge:
...
With that said, some values of k make more sense than others.

For instance, imagine if this was our mystery point:
...
We're going to come back to this idea of higher-dimensional space next week
(sorry). 

For now, let's move on to  actually fitting KNNs in R. We'll be working with
our attrition data to start off with -- go ahead and load it now, then create
our testing and training splits:
...
Before we fit our model, we have to do a bit of data cleaning. 

Our attrition data contains a bunch of numeric variables in it. For instance,
it's got data on the `Age` and `DailyRate` of employees:
...
So in order for KNN to treat all our variables equally, we should center and
scale them.
...
fitting a model -- whether it's filtering out variables, rescaling data,
...
And now we're finally able to get started fitting our model!
...
before predicting. `knn3` doesn't take weights as an argument, so we'll need
...
means that cross-entropy decreases as k increases and guesses become less 
confident, so can't actually tell us what the best k value might be.
...
Note that we're going to scale our data during each iteration -- just like with
the hold-out set, it's important we not let data from the validation fold leak 
into the pre-processing step:
...
Plotting k makes it pretty obvious that the optimal value is somewhere just 
under 100.
...
Not too bad! We've done dramatically better with random forests and GBMs, but
...
+ [HOML Chapter 8](https://bradleyboehmke.github.io/HOML/knn.html)
