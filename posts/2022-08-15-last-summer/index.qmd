...
I'm wrapping up three months as an open-source intern with [Posit](https://posit.co/) (née [RStudio](https://www.rstudio.com/)). I wanted to write a small post talking a bit about what it was like and what we got done in that timespan. So, well, this is that post[^1].
...
That meant a pretty huge update to the package, and that version is [on CRAN now](https://www.tidyverse.org/blog/2022/06/spatialsample-0-2-0/). A small patch is coming in the next few weeks to speed up buffering, work better with simulated data, and make me the official package maintainer as Julia moves to focus more on ModelOps.
...
I'm particularly excited for the grouped resampling functions to hit CRAN, since I think those will solve a very common use-case for a lot of people working with repeated measures data.
...
* Published ["Filtering ground noise from LiDAR returns produces inferior models of forest aboveground biomass in heterogenous landscapes"](https://doi.org/10.1080/15481603.2022.2103069) in GIScience and Remote Sensing^[This link doesn't work at the time of publication, but will _eventually_ point to the article once the online version gets published.]. 
